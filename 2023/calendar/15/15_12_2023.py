...
approach the mountain, you find that the light is being collected by the wall of a
...
goggles and a loose-fitting hard hat. The reindeer leads you to a shelf of goggles and
hard hats (you quickly find some that fit) and then further into the facility. At one
point, you pass a button with a faint snout mark and the label "PUSH FOR HELP". No
...
large assortment of lenses available to further focus the light, the reindeer brings
you a book titled "Initialization Manual".
...
the range 0 to 255. To run the HASH algorithm on a string, start with a current value
...
The initialization sequence (your puzzle input) is a comma-separated list of steps to
start the Lava Production Facility. Ignore newline characters when parsing the
...
the results? (The initialization sequence is one long line; be careful when
copy-pasting it.)
...
organized by focal length ranging from 1 through 9. The reindeer also brings you a
small handheld label printer.

The book goes on to explain how to perform each step in the initialization sequence,
a process it calls the Holiday ASCII String Helper Manual Arrangement Procedure, or
HASHMAP for short.

Each step begins with a sequence of letters that indicate the label of the lens on
which the step operates. The result of running the HASH algorithm on the label
...
to use the label maker to mark the lens with the label given in the beginning of the
...
immediately behind any lenses already in the box. Don't move any of the other lenses
...
here. Within each box, lenses are listed from front to back; each lens is shown as its
label and focal length in square brackets.
...
class Box:
    ...
    def add_lens(self, lens: Lens) -> None:
        # If the lens is already in the box, replace it
    ...
    def score(self) -> int:
        ...
        return sum(
            [
                (self.number + 1) * (i + 1) * l.focal_length
                for i, l in enumerate(self.lenses)
            ]
        )
...
class BoxCollection:
    ...
    def remove_lens(self, label: str, box_number: int) -> None:
        self.boxes[box_number].remove_lens(label)

    def __repr__(self) -> str:
        return "\n".join([str(b) for b in self.boxes])

    def score(self) -> int:
        return sum([b.score() for b in self.boxes])
...
def part_one(data: List[str]) -> int:
    inputs = data[0].split(",")
    score = sum([decode(i) for i in inputs])
    return score
...
def part_two(data: List[str]) -> int:
    ...
    for i in inputs:
        if "=" in i:
            ...
            box_number = decode(label)
            focal_length = int(focal_length)
            lens = Lens(label, focal_length)
            box_collection.add_lens(lens, box_number)
        elif "-" in i:
            label = i.split("-")[0]
            box_number = decode(label)
            box_collection.remove_lens(label, box_number)
    return box_collection.score()
...
if __name__ == "__main__":
    ...
    TEST_DATA: List[str] = ["rn=1,cm-,qp=3,cm=2,qp-,pc=4,ot=9,ab=5,pc-,pc=6,ot=7"]

    # Starting Part One at 10:01AM CST

    PART_ONE_EXPECTED_VALUE: int = 1320
    ...
    PART_TWO_EXPECTED_VALUE: int = 145
