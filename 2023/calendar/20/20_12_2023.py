...
b, and c. Each of these modules is a flip-flop module (as indicated by the % prefix).
a outputs to b which outputs to c which outputs to another module named inv. inv is a
conjunction module (as indicated by the & prefix) which, because it has only one input,
acts like an inverter (it sends the opposite of the pulse type it receives); it
outputs to a.
...
class Pulse:
    ...
    def __repr__(self) -> str:
        return (
            f"[{self.button_press}] {self.source} "
            f"---{self.signal}--> {self.destination}"
        )
...
class Module:
    ...
    def ping(self, pulse: Pulse) -> List[Pulse]:
        pass

    def __repr__(self) -> str:
        return f"{type(self)} {self.name} -> {self.destination_modules}"
...
class FlipFlop(Module):
    ...
    def ping(self, pulse: Pulse) -> List[Pulse]:
        if pulse.signal == Signal.LOW:
            if self._state == FlipFlopState.OFF:
                self.toggle()
                return [
                    Pulse(self.name, d, Signal.HIGH, pulse.button_press)
                    for d in self.destination_modules
                ]
            elif self._state == FlipFlopState.ON:
                self.toggle()
                return [
                    Pulse(self.name, d, Signal.LOW, pulse.button_press)
                    for d in self.destination_modules
                ]
        return []
...
class Conjunction(Module):
    def __init__(self, name: str, destination_modules: List[str]) -> None:
        super().__init__(name, destination_modules)
        self.memory: DefaultDict[str, Signal] = DefaultDict(lambda: Signal.LOW)

    def ping(self, pulse: Pulse) -> List[Pulse]:
        self.memory[pulse.source] = pulse.signal
...
class Broadcast(Module):
    def ping(self, pulse: Pulse) -> List[Pulse]:
        return [
            Pulse(self.name, d, pulse.signal, pulse.button_press)
            for d in self.destination_modules
        ]
...
class Output(Module):
    def ping(self, pulse: Pulse) -> List[Pulse]:
        if pulse.signal == Signal.LOW and self.name == "rx":
            raise ValueError("Output modules cannot receive pulses")
        return []
...
class CommunicationModule:
    def __init__(self, data: List[str], part: int = 1) -> None:
        self._data = data
        self._modules: Dict[str, Module] = self._parse_data(self._data)
        self.low_counter: int = 0
        self.high_counter: int = 0
        self.part = part

    def _parse_data(self, data: List[str]) -> Dict[str, Module]:
        modules: DefaultDict[str, Module] = DefaultDict(lambda: Output("", []))
        for line in data:
            ...
            if name.startswith("%"):
                ...
                modules[name] = Output(name, destinations)
            else:
                raise ValueError(f"Unknown module type: {name}")

        # Add these two badboys
        name = "button"
        destinations = ["broadcaster"]
        modules[name] = Button(name, destinations)
        name = "output"
        destinations = []
        modules[name] = Output(name, destinations)

        # Initialize the memory of all conjunctions
        conjunctions = [m.name for m in modules.values() if isinstance(m, Conjunction)]
        for m in modules.values():
            for d in m.destination_modules:
                if d in conjunctions:
                    modules[d].memory[m.name] = Signal.LOW

        return modules

    def press_button(self, n: int = 1) -> None:

        zr_memory: Dict[str, int] = {"sz": None, "cm": None, "xf": None, "gc": None}
        for i in range(1, n):
            pulses = self._modules["button"].ping(
                Pulse("finger", "button", Signal.LOW, i)
            )
            while pulses:
                pulse = pulses.pop(0)
                if pulse.signal == Signal.LOW:
                    self.low_counter += 1
                elif pulse.signal == Signal.HIGH:
                    self.high_counter += 1
                try:
                    new_pulses = self._modules[pulse.destination].ping(pulse)
                    if self.part == 2:
                        module = self._modules["zr"]
                        for m, mem in module.memory.items():
                            if mem == Signal.HIGH and zr_memory[m] is None:
                                zr_memory[m] = pulse.button_press
                        if all(v is not None for v in zr_memory.values()):
                            ret_val = lcm([v for v in zr_memory.values()])
                            print(f"Part 2: {ret_val} button presses")
                            return ret_val
                except ValueError:
                    if self.part == 2:
                        print(f"Part 2: {i} button presses")
                    else:
                        new_pulses = []
                pulses += new_pulses
...
def part_one(data: List[str]) -> int:
    cm = CommunicationModule(data)
    cm.press_button(1000)
    return cm.score()


def part_two(data: List[str]) -> int:
    cm = CommunicationModule(data, part=2)
    cm.press_button(1_000_000_000)
    return None
...
if __name__ == "__main__":
    ...
    # Starting Part One at 7:38AM CST

    PART_ONE_EXPECTED_VALUE: int = 32000000
    PART_ONE_EXPECTED_VALUE_TWO: int = 11687500
    ...
    # Completed Part One at 8:40AM CST

    # Starting Part Two at 8:44AM CST
