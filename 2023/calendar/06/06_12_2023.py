...
class Race:
    ...
    def calculate_distances(self) -> List[int]:
        # D = TIME * time_held - time_held^2
        return [
            self.time * time_held - time_held**2 for time_held in range(self.time)
        ]

    @property
    def winning_scenarios(self) -> int:
        return sum([int(d > self.distance) for d in self.calculate_distances()])


def part_one(data: List[str]) -> int:
    race_times = [int(time) for time in data[0].split()[1:]]
    race_distances = [int(distance) for distance in data[1].split()[1:]]
    races: List[Race] = [Race(t, d) for t, d in zip(race_times, race_distances)]
    score = reduce(mul, [r.winning_scenarios for r in races])
    return score


def part_two(data: List[str]) -> int:
    race_time = int(data[0].replace(" ", "").split(":")[1])
    race_distance = int(data[1].replace(" ", "").split(":")[1])
    race = Race(time=race_time, distance=race_distance)
    return race.winning_scenarios
...
if __name__ == "__main__":
    ...
    PART_ONE_EXPECTED_VALUE = 288

    # Starting Part One at 8:25AM CST

    print(f"Part One: {part_one(TEST_DATA)} (expected {PART_ONE_EXPECTED_VALUE})")
    print(f"Part One: {part_one(DATA)}")

    # Completed Part One at 8:37AM CST

    # Starting Part Two at 8:39AM CST

    PART_TWO_EXPECTED_VALUE = 71503
    print(f"Part Two: {part_two(TEST_DATA)} (expected {PART_TWO_EXPECTED_VALUE})")
    print(f"Part Two: {part_two(DATA)}")

    # Completed Part Two at 8:45AM CST
